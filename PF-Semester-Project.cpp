...
void load_data() {
    ...
    for (int i = 0; i < student_count; i++) {
        student_file >> student_roll[i];
        student_file.ignore(); // Ignore newline
        student_file.getline(student_name[i], MAX_NAME_LENGTH);
        student_file >> student_balance[i];
    }

    // Load book data
    book_file >> book_count;
    for (int i = 0; i < book_count; i++) {
        book_file.ignore(); // Ignore newline
        book_file.getline(book_title[i], MAX_NAME_LENGTH);
        book_file.getline(book_author[i], MAX_NAME_LENGTH);
        book_file >> book_isbn[i];
        book_file >> book_available[i];
    }
...
void create_account() {
    ...
    if (cin.fail() || initial_deposit < 50) {
        cin.clear();
        cin.ignore(numeric_limits<streamsize>::max(), '\n');
        cout << "Initial deposit must be at least $50.\n";
        return;
...
// Function to display student account details
void display(int roll) {
    int index = find_student(roll);
    if (index == -1) {
        cout << "Student not found.\n";
...
// Function to deposit amount into student account
void deposit_amount(int roll, double amount) {
    int index = find_student(roll);
    if (index == -1) {
        cout << "Student not found.\n";
...
// Function to display sorted list of students by roll number
void display_sorted() {
    for (int i = 0; i < student_count; i++) {
        for (int j = i + 1; j < student_count; j++) {
            if (student_roll[i] > student_roll[j]) {
